...
const AbstractCore = () => {
  ...
  useFrame((state) => {
    ...
    if (materialRef.current) {
      materialRef.current.distort = THREE.MathUtils.lerp(
        ...
        0.35 + Math.sin(state.clock.elapsedTime * 0.8) * 0.1,
        0.05
      );
    }
  });
  ...
  return (
    <Float speed={1.8} rotationIntensity={0.6} floatIntensity={1.2}>
      ...
      {/* Outer Geometric Wireframe Layer */}
      <mesh scale={2.8}>
...
const Particles = () => {
  ...
  return (
    <points ref={pointsRef}>
      ...
      <pointsMaterial size={0.03} color="#fb8dff" transparent opacity={0.45} sizeAttenuation />
    </points>
  );
};
